import React from "react";
import {Link} from "react-router-dom";
import {Button, Card} from "antd";
import {Meta} from "antd/es/list/Item";
import {useDispatch, useSelector} from "react-redux";
import {getId, getIsAuth} from "../../state/auth-selectors";
import {createDialog} from "../../state/dialogs-reducer";
import Preloader from "../../common/Preloader/Preloader";
type UserPropsType={
    id:number,
    photos:{
        small:string | null,
        large:string | null
    }
    followingInProgress:Array<number>,
    follow:(id:number)=>void,
    unFollow:(id:number)=>void,
    followed:boolean,
    name:string,
    status:string | null
}
const User:React.FC<UserPropsType>=({id,photos,status,name,followingInProgress,follow,followed,unFollow})=>{
const isAuth=useSelector(getIsAuth);
const myId=useSelector(getId);
const dispatch=useDispatch();
    const isFollowing:boolean=!!followingInProgress.includes(id);
const createDialogHandler=()=>dispatch(createDialog(id));
    return (
        <Card
            style={{ width: 300,margin:"0 10px 10px 0"}}
            cover={
               <Link to={`/profile/${id}`}>
                   <img
                       style={{width:300,height:300}}
                       alt="example"
                       src={photos.large ? photos.large:"data:image/png;base64," }
                   />
               </Link>
            }
            actions={[
             myId!==id &&  isAuth ? followed ? <Button disabled={isFollowing} style={{width:"90%"}} onClick={()=>unFollow(id)}>{isFollowing ? <Preloader fontSize={20}/> : "unFollow"}</Button>
                    :<Button disabled={isFollowing} style={{width:"90%"}}
                             type={isFollowing  ? "default" :"primary"} onClick={()=>follow(id)}> {isFollowing ? <Preloader fontSize={20}/> : "Follow"}</Button>:"",
                myId!==id &&  isAuth &&    <Link to={`/dialogs/${id}`}>
                <Button onClick={createDialogHandler} style={{width:"90%"}} type={"primary"}>Написать</Button>
            </Link>
            ]}
        >
            <Meta
                title={name}
                description={status ? status :"Status is missing"}
            />
        </Card>
    )
}
export default User
